...
class FactoryTest extends TestCase
{
    ...
    public function testCreateWithDependencies()
    {
        ...
        $cache->shouldReceive("has")->with("http://192.168.4.1:1400/test.xml")->once()->andReturn(true);
        $logger->shouldReceive("info")->with("getting xml from cache: http://192.168.4.1:1400/test.xml")->once()->andReturn(true);
        $cache->shouldReceive("get")->with("http://192.168.4.1:1400/test.xml")->once()->andReturn("<test>ok</test>");
